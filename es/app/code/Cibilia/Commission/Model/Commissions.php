...
class Commissions extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function _genrateLockedCommission($oid){
	        ...
	        foreach ($arrCibiliansTotals as $cibilianOrder) {
	            try {
					...
					//$this->_updateCibilianSummary($modelCommission);
					$this->_checkCibilianSummary($modelCommission);
    ...
    public function _updateLockedCommission(){
       ...
       // $lastUpdateDate = date('Y-m-d H:i:s', strtotime('-2 day'));
        ...
        if(count($lockedCommission) > 0){
        	...
        	$lockedOrders = array();
        	$lockedOrdersCommission = array();
        	
        	foreach ($lockedCommission as $oid) {
        		$lockedOrders[$oid['order_id']] =  $oid['order_id'];
        		$lockedOrdersCommission[$oid['order_id']]['commission'][] =  $oid['commission_id'];
        	}
        	...
        	foreach ($arrOrdersCollection as $order) {
        		foreach ($lockedOrdersCommission[$order->getId()]['commission'] as $cid) {
        			
        			$objCommission = $this->_objectManager->create('Cibilia\Commission\Model\Commissions')->load($cid);
        			if($objCommission && $objCommission->getId()){

        				$objCommission->setStatus(2);

						$objCommission->save();

						$this->_updateCibilianSummary($objCommission);
    ...
    public function _checkCibilianSummary($commission){
