...
class RefundRows extends \Magento\Backend\Block\Widget\Grid\Extended implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    {
        $payout = $this->_registry->registry('payout_data');
        if (!$payout) {
            $payout = $this->_registry->create()->load($this->getPayoutId());
            $this->_registry->register('payout_data', $payout);
        }
        return $payout;
    }
    ...
    {
        /** @var \Unirgy\DropshipPayout\Model\ResourceModel\Payout\RefundRow\Collection $collection */
        $collection = $this->_hlp->createObj('\Unirgy\DropshipPayout\Model\Payout\RefundRow')->getCollection()
